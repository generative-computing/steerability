...
class ContrastiveDecoding(OutputControl):
    ...
    Args:
        amateur_name_or_path (str): HF hub id or local path for the amateur LM.
        alpha (float): Plausibility-mask threshold in `[0, 1]`. Defaults to 0.1.
        base_weight (float): Weight on the base (expert) log-probs. Defaults to 1.0.
        amateur_weight (float): Weight subtracted for the amateur log-probs. Defaults to 1.0.
        hf_model_kwargs (dict): Extra kwargs passed to `AutoModelForCausalLM.from_pretrained()` for
            the amateur model. Defaults to {}.

"""Opt-in verbosity controls for the `aisteer360` package logger.
...
_ROOT_LOGGER_NAME = "aisteer360"
_ENV_VAR = "AISTEER_VERBOSITY"
...
def set_verbosity(level: int | str) -> None:
    """Set the level of the `aisteer360` logger and attach one stream handler if none is attached.

    Sets the level of the package root logger (`aisteer360`), so every module logger under it is
...
def get_verbosity() -> int:
    """Return the effective level of the `aisteer360` logger.

    Applies the `AISTEER_VERBOSITY` environment default once (on first library use) before
...
def _apply_env_default() -> None:
    """Apply the `AISTEER_VERBOSITY` level once if it is set and no explicit call has been made.

    Reads the environment on first invocation. If `AISTEER_VERBOSITY` is set to a recognized level
    ...
    rather than at import time, so importing `aisteer360` never inspects the environment.
    """
    global _env_default_applied

...
def _constant_scorer(value: float = 0.5):
    """Trivial per-row scorer returning the same scalar regardless of input. Used for fast smoke tests."""
...
class TestPRewriteArgs:
    ...
    def test_search_requires_devset(self):
        with pytest.raises(ValueError, match="dev_set"):
            PRewriteArgs(initial_instruction="x", strategy="search", row_scorer=_constant_scorer())

    def test_search_requires_row_scorer(self):
    ...
    def test_train_accepts_scorer_devset_reward(self):
        args = PRewriteArgs(
            initial_instruction="x",
            strategy="inference",
            train_rewriter=True,
            rewriter_model_name_or_path="some/rewriter",
            row_scorer=_constant_scorer(),
            dev_set=[{"input": "a"}],
        )
...
class TestPRewriteSearchStrategy:
    def test_runs_end_to_end(self, model_and_tokenizer, device: torch.device):
        ...
        prewrite = PRewrite(
            ...
            row_scorer=_constant_scorer(value=1.0),

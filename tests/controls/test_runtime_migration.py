"""Post-migration invariants for the runtime-backed state controls (ITI, AngularSteering).

Complements the golden and anti-drift tests with structural checks on the shared
`TransformHookRuntime`: exactly one pass opener per generation, the shared offset advances once
per forward pass to `prompt_len + decode_passes`, and the single-forward (`compute_logprobs`)
path steers the expected scope.

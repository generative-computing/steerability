...
def _build_refs_list(
    ...
    """Validate a list of fixed reference pipelines.

    Each entry is validated to contain at most one configuration.
            ...
            if ref_df is not None and not ref_df.empty and "config_id" in ref_df.columns:
                ...
                if n_configs > 1:
                    raise ValueError(
                        ...
                        f"configurations. Only fixed (non-swept) pipelines are supported; "
                        f"plot ControlSpec sweeps with multiple configurations as a "
                        f"separate swept series."
                    )
    ...
    if baseline_value is not None:
        ...
        if baseline_std is not None:
            ax.axhspan(baseline_value - baseline_std,
                        ...
                        facecolor="#999999", alpha=0.1, edgecolor="none", zorder=0)
...
def plot_tradeoff_scatter(
    ...
    **scatter_kwargs: Any,
    ...
    markers using `compare_to_pipelines`.  A Pareto frontier can optionally be
    shown.
    ...
    all_refs = _build_refs_list(compare_to_pipelines)
...
def create_tradeoff_figure(
    ...
    refs = [(baseline_pipeline, baseline)] if not baseline.empty else []

    fig, axes = plt.subplots(1, 3, figsize=figsize)

    plot_sensitivity(
        swept, metric=x_metric, sweep_col=sweep_col, compare_to_pipelines=refs,
        ax=axes[0], metric_label=x_label, sweep_label=sweep_label,
        title=f"{x_label} sensitivity",
    )
    plot_sensitivity(
        swept, metric=y_metric, sweep_col=sweep_col, compare_to_pipelines=refs,
    ...
    plot_tradeoff(
        ...
        compare_to_pipelines=refs, ax=axes[2], x_label=x_label, y_label=y_label,

...
on first resolution, so `core` carries no module-level dependency on `steerability.backends`.
"""
...
_BACKEND_CLASSES: dict[str, tuple[str, str]] = {
    "huggingface": ("steerability.backends.huggingface", "HFBackend"),
    "vllm": ("steerability.backends.vllm", "VLLMBackend"),
    "vllm-serve": ("steerability.backends.vllm", "VLLMServeBackend"),
}
